...
function optimize_many_MAP(model, n_reps = 100, top_n = 1, verbose = true)
  ...
  for i in eachindex(lp_res)
      ...
      Random.seed!(i)
      try
          lp_res[i] = optimize(model, MAP(), LBFGS(linesearch = LineSearches.BackTracking()), Optim.Options(iterations=5_000, allow_f_increases=true)).lp
          print(lp_res[i])
      catch
      end
  end
  ...
  map(best_n_seeds) do seed
    Random.seed!(seed)
    optimize(model, MAP(), LBFGS(linesearch = LineSearches.BackTracking()), Optim.Options(iterations=5_000, allow_f_increases=true)).values.array
  end
end
