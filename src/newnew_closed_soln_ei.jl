...
function newnew_ei_closed_solution!(outs_tmp, times, change_times, grid_size, t0, alphas, init_conds, gamma, nu) 
  num_alphas = length(alphas)
  # if num_alphas==1, then then stop_times is only the end
  # and start_times is only the start
  if num_alphas==1
  ...
  for i in 1:num_alphas
    ...
    current_init_conds = outs_matrix[2:3,round(Int64, maximum(new_indices))]
