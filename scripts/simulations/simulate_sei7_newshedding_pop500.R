...
while (i <= num_sims) {
  ...
  potential_epidemic = sim_SEIIIIIIIRRR_nonconst_new(N, 
                                                     ...
                                                     eta2 = eta,
                                                     t_stop)
  min_step = potential_epidemic[[2]] %>% filter(time > 0) %>% pull(time) %>% min()
  if (min_step < 1) {
...
huhhuh = epidemics[[26]][[2]] %>%
  mutate(total_shed = I1 + I2 + I3 + I4 + I5 + I6 + I7 + R1 + R2) %>%
  filter(time > 0)
...
write_csv(full_obs_data, here::here("data", "newshedding_data", paste0("pop500_fulldata.csv")))
write_csv(final_obs_data, here::here("data", "newshedding_data", paste0("pop500_data.csv")))

sim_key <- read_csv(here::here("data", "newshedding_data", "sim_key.csv"))

key_frame <- data.frame(sim_num = 106,
                      sim_name = "newshedding_pop500",
                      sim_filename = "simulate_sei7_newshedding_pop500.R",
                      obsdata_filename = "data/newshedding_data/pop500_data.csv",
                      fulldata_filename = "data/newshedding_data/pop500_fulldata.csv",
                      N = N,
                      E_init = E_init,
                      gamma = gamma,
                      nu = nu,
