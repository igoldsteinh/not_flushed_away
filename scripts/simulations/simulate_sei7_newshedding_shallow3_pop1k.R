...
plot_states <- state_data[[1]] %>% 
  filter(time <= 7 * 14) %>%
  dplyr::select(time, E, I1, I2, I3, I4, I5, I6, I7) %>%
  pivot_longer(-time) %>%
...
write_csv(full_obs_data, here::here("data", "newshedding_data", paste0("pop1000_shallow3_fulldata.csv")))
write_csv(final_obs_data, here::here("data", "newshedding_data", paste0("pop1000_shallow3_data.csv")))

sim_key <- read_csv(here::here("data", "newshedding_data", "sim_key.csv"))

key_frame <- data.frame(sim_num = 115,
                      sim_name = "newshedding_pop1000_shallow3",
                      sim_filename = "simulate_sei7_newshedding_shallow3_pop1k.R",
                      obsdata_filename = "data/newshedding_data/pop1000_shallow3_data.csv",
                      fulldata_filename = "data/newshedding_data/pop1000_shallow3_fulldata.csv",
                      N = N,
                      E_init = E_init,
                      gamma = gamma,
                      nu = nu,
                      eta = eta,
                      r0 = paste(as.character(r0_vec), collapse = ","),
                      change_points = paste(as.character(change_points), collapse = ","))
