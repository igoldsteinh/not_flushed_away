...
# new shedding 
# NOTE
# there is going to be a problem with calculating time since infectious
# for individuals starting in the I2 - I4 compartments
# we will have to be careful about retroactively giving them an infectious time
# to get around this, for now only simulate with people starting in I1 compartment
library(tidyverse)
library(glmnet)
library(patchwork)
library(GGally)
...
r03 = 2 # then a five week jump up to 1.8
r0_init = r03[1]
r0_vec = r03
unique_r0 = r03
...
times = map(state_data, ~seq(0, max(.x$integer_day), by = 0.25))
...
write_csv(true_conc_data %>% bind_rows(), here::here("data", "newshedding_data", "example_shedding.csv"))
# plot the concentration
plot_conc = true_conc_data %>% 
  bind_rows() %>%
  group_by(sim) %>%
