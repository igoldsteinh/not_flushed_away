...
G_data <- read_csv(here::here("data", "G_case_data_trunc.csv"))
...
init_rt_prior <- EpiNow2::rt_opts(
  prior = EpiNow2::LogNormal(mean = 0.5, sd = 0.1)
)
ls_mn_sd = c(6 * 7, 7)
...
en2_args <- list(
  ...
  delays = gamma_delay,
  obs = ascertainment,
  rt = rt_opts(prior = LogNormal(mean = 0.5, sd = 0.1), rw = 1),
  gp = NULL,
  stan = stan_opts,
  forecast = NULL,
  CrIs = c(0.5, 0.8, 0.95)
...
write_csv(R_summaries, here::here("results", "epinow2", "epinow2_G_trunc_rt0.5_rw.csv"))


# E data ------------------------------------------------------------------

E_data <- read_csv(here::here("data", "E_case_data_trunc.csv")) %>%
...
R_summaries <- summaries %>% filter(variable == "R") %>%
  left_join(date_crosswalk, by = "date")
write_csv(R_summaries, here::here("results", "epinow2", "epinow2_E_trunc_rt0.5_rw.csv"))
