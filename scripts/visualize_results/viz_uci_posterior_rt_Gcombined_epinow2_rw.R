...
# case data
G_cases = read_csv(here::here("data", "G_case_data_corrected.csv")) %>%
  mutate(Community = "G")

E_cases = read_csv(here::here("data", "E_case_data_corrected.csv")) %>%
  mutate(Community = "E")
...
# ggsave(here::here("figures", "uci_data_plot_corrected.pdf"), uci_data_plot, height = 10, width = 10)
...
G_case_posteriort = read_csv(here::here("results", 
                                         "epinow2", 
                                         "epinow2_G_rt0.5_rw.csv"))
G_case_95 <- G_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_95, upper_95) %>%
  rename(.lower = lower_95, 
         .upper = upper_95) %>%
  mutate(.width = 0.95)
G_case_80 <- G_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_80, upper_80) %>%
  rename(.lower = lower_80, 
         .upper = upper_80) %>%
  mutate(.width = 0.8)
G_case_50 <- G_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_50, upper_50) %>%
  rename(.lower = lower_50, 
         .upper = upper_50) %>%
  mutate(.width = 0.5)

G_case_posteriort_plotdata <- G_case_95 %>%
  bind_rows(G_case_80) %>%
  bind_rows(G_case_50)

G_case_rt_plot <- G_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
  ggtitle("Epinow2-cases G") + 
  my_theme + 
  ylim(c(0,3.5)) +
  ylab("") + 
  xlab("") + 
...
E_case_posteriort = read_csv(here::here("results", 
                                        "epinow2", 
                                        "epinow2_E_rt0.5_rw.csv"))
E_case_95 <- E_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_95, upper_95) %>%
  rename(.lower = lower_95, 
         .upper = upper_95) %>%
  mutate(.width = 0.95)
E_case_80 <- E_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_80, upper_80) %>%
  rename(.lower = lower_80, 
         .upper = upper_80) %>%
  mutate(.width = 0.8)
E_case_50 <- E_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_50, upper_50) %>%
  rename(.lower = lower_50, 
         .upper = upper_50) %>%
...
case_extra_rows <- extra_rows %>%
  dplyr::select(date, value, .lower, .upper, .width) %>%
  rename(real_date = date)
E_case_posteriort_plotdata <- E_case_95 %>%
  bind_rows(E_case_80) %>%
  bind_rows(E_case_50) %>%
  bind_rows(case_extra_rows)

E_case_rt_plot <- E_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
  ggtitle("Epinow2-cases E") + 
  my_theme + 
  ylim(c(0,3.5)) +
  ylab("") + 
  xlab("") + 
...
E_case_rt_plot_bottom <- E_case_posteriort %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
...
# ggsave(here::here("figures", "uci_all_posterior_fig_epinow2_rw.pdf"), episewer_posterior_grid, height = 15, width = 15)
...
G_case_rt1_plot <- G_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
...
E_case_rt1_plot <- E_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
...
all_posterior_fig_rt1 <-  (E_rt1_det_plot_posterior_rt +  E_rt1_stc_plot_posterior_rt + E_case_rt1_plot)/
  (G_rt1_det_plot_posterior_rt + G_rt1_stc_plot_posterior_rt + G_case_rt1_plot) + plot_annotation(
    title = 'Estimated Rt from Wastewater and Cases Initial Rt ~ 1'
  ) &
  theme(text = element_text(size = 18))

# ggsave(here::here("figures", "uci_all_posterior_rt=1_epinow2_fig.pdf"), all_posterior_fig_rt1, height = 12, width = 12)

# rt=0.75 --------------------------------------------------------------------

# G1 ---------------------------------------------------------------------
seed_val = 97
G_rt0.75_stc_timevarying_quantiles <- read_csv(here::here("results", 
                                                       "stoch_ei_normal", 
                                                       "generated_quantities", 
                                                       paste0("posterior_timevarying_quantiles_scenario", 
                                                              "Gcombined_rt0.75", "_seed", seed_val, ".csv")),
                                            show_col_types = FALSE) 


G_rt0.75_stc_rt_quantiles <- G_rt0.75_stc_timevarying_quantiles %>%
  filter(name == "rt_t_values") %>%
  rename(lump = time) %>% 
  dplyr::select(lump, value, .lower, .upper, .width,.point, .interval) %>%
  right_join(G_time_crosswalk, by = c("lump")) %>%
...
# visualize results
G_rt0.75_stc_plot_posterior_rt <- G_rt0.75_stc_rt_quantiles %>% 
  filter(lump >= 0) %>%
  ggplot(aes(x = date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
...
# redo for det
seed_val = 1
G_rt0.75_det_timevarying_quantiles <- read_csv(here::here("results", 
                                                       "det_ei_normal", 
                                                       "generated_quantities", 
                                                       paste0("posterior_timevarying_quantiles_scenario", 
                                                              "Gcombined_rt0.75", "_seed", seed_val, ".csv")),
...
G_rt0.75_det_rt_quantiles <- G_rt0.75_det_timevarying_quantiles %>%
  filter(name == "rt_t_values") %>%
  rename(lump = time) %>% 
  dplyr::select(lump, value, .lower, .upper, .width,.point, .interval) %>%
  right_join(G_time_crosswalk, by = c("lump")) %>%
...
# visualize results
G_rt0.75_det_plot_posterior_rt <- G_rt0.75_det_rt_quantiles %>% 
  filter(lump >= 0) %>%
  ggplot(aes(x = date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
...
# E ------------------------------------------------------------------
seed_val = 97
E_rt0.75_stc_timevarying_quantiles <- read_csv(here::here("results", 
                                                       "stoch_ei_normal", 
                                                       "generated_quantities", 
                                                       paste0("posterior_timevarying_quantiles_scenario", 
                                                              "E_rt0.75", "_seed", seed_val, ".csv")),
                                            show_col_types = FALSE) 


E_rt0.75_stc_rt_quantiles <- E_rt0.75_stc_timevarying_quantiles %>%
  filter(name == "rt_t_values") %>%
  rename(lump = time) %>% 
  dplyr::select(lump, value, .lower, .upper, .width,.point, .interval) %>%
  right_join(E_time_crosswalk, by = c("lump")) %>%
...
extra_rows <- G_rt0.75_stc_rt_quantiles %>% 
  filter(date == min(date))
...
E_rt0.75_stc_rt_quantiles <- bind_rows(extra_rows, E_rt0.75_stc_rt_quantiles)
# visualize results
E_rt0.75_stc_plot_posterior_rt <- E_rt0.75_stc_rt_quantiles %>% 
  filter(lump >= 0) %>%
  ggplot(aes(x = date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
...
# redo for det
seed_val = 1
E_rt0.75_det_timevarying_quantiles <- read_csv(here::here("results", 
                                                       "det_ei_normal", 
                                                       "generated_quantities", 
                                                       paste0("posterior_timevarying_quantiles_scenario", 
                                                              "E_rt0.75", "_seed", seed_val, ".csv")),
...
E_rt0.75_det_rt_quantiles <- E_rt0.75_det_timevarying_quantiles %>%
  filter(name == "rt_t_values") %>%
  rename(lump = time) %>% 
  dplyr::select(lump, value, .lower, .upper, .width,.point, .interval) %>%
  right_join(E_time_crosswalk, by = c("lump")) %>%
...
E_rt0.75_det_rt_quantiles <- bind_rows(E_rt0.75_det_rt_quantiles, extra_rows)
# visualize results
E_rt0.75_det_plot_posterior_rt <- E_rt0.75_det_rt_quantiles %>% 
  filter(lump >= 0) %>%
  ggplot(aes(x = date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
...
E_rt0.75_stoch_vs_det <- E_rt0.75_det_plot_posterior_rt + E_rt0.75_stc_plot_posterior_rt + plot_layout(guides = "collect")

# epidemia case fits ------------------------------------------------------
# G  --------------------------------------------------------------
G_case_posteriort = read_csv(here::here("results", 
                                        "epinow2", 
                                        "epinow2_G_rt0.75_rw.csv"))
G_case_95 <- G_case_posteriort %>%
  rename(value = median) %>%
  dplyr::select(real_date, value, lower_95, upper_95) %>%
  rename(.lower = lower_95, 
...
G_case_rt0.75_plot <- G_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
...
E_case_rt0.75_plot <- E_case_posteriort_plotdata %>% 
  ggplot(aes(x = real_date, y = value,  ymin = .lower, ymax = .upper)) +
  geom_lineribbon() +
  geom_hline(yintercept = 1, linetype = "dashed") +
  theme_bw() +
...
all_posterior_fig_rt0.75 <-  (E_rt0.75_det_plot_posterior_rt +  E_rt0.75_stc_plot_posterior_rt + E_case_rt0.75_plot)/
  (G_rt1_det_plot_posterior_rt + G_rt0.75_stc_plot_posterior_rt + G_case_rt0.75_plot) + plot_annotation(
    title = 'Estimated Rt from Wastewater and Cases Initial Rt ~ 0.75'
  ) &
  theme(text = element_text(size = 18))

# ggsave(here::here("figures", "uci_all_posterior_rt=0.75_epinow2_fig.pdf"), all_posterior_fig_rt0.75, height = 12, width = 12)
