...
# visualize uci data ------------------------------------------------------
# ww data
E_data_trunc <- read_csv(here::here("data", "E_data_trunc.csv"), 
                     show_col_types = FALSE)
Gcombined_data_trunc <- read_csv(here::here("data", "Gcombined_data_trunc.csv")) %>%
  mutate(place = "G") %>%
...
uci_plot_data <- E_data_trunc %>%
  bind_rows(Gcombined_data_trunc) %>%
...
# case data
G_cases_trunc = read_csv(here::here("data", "G_case_data_trunc.csv")) %>%
  mutate(Community = "G")

E_cases_trunc = read_csv(here::here("data", "E_case_data_trunc.csv")) %>%
  mutate(Community = "E") %>%
  filter(date > as.Date("2022-02-01"))


weekly_cases = bind_rows(G_cases_trunc, E_cases_trunc)
...
# ggsave(here::here("figures", "uci_data_plot_trunc.pdf"), uci_data_plot, height = 10, width = 10)


# rt at 0.5 ww models -----------------------------------------------------
# create time crosswalks for visualizing compartment counts
Gcombined_data <- read_csv(here::here("data", "Gcombined_data_trunc.csv")) %>%
  mutate(place = "G") %>%
  dplyr::select(place, date, new_time, log_mean_conc) %>%
  rename(log_conc = log_mean_conc)
...
E_data <- read_csv(here::here("data", "E_data_trunc.csv"), 
                   show_col_types = FALSE)
...
G_stc_timevarying_quantiles <- read_csv(here::here("results", 
                                                     ...
                                                     paste0("posterior_timevarying_quantiles_scenario", 
                                                            "Gcombined_trunc", "_seed", seed_val, ".csv")),
                                          show_col_types = FALSE) 
...
episewer_E <- read_rds(here::here("results", "episewer", "episewer_E_trunc.rds"))
episewer_G <- read_rds(here::here("results", "episewer", "episewer_G_trunc.rds"))
...
# ggsave(here::here("figures", "uci_all_posterior_fig_epinow2_rw_trunc.pdf"), episewer_posterior_grid, height = 15, width = 15)
