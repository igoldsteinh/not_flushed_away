...
timevarying_suffix <- paste0("posterior_timevarying_quantiles_scenario",
                             sim)
timevarying_list <- list.files(path = path("results", "stoch_conc_ei_normal", "generated_quantities"),  pattern = timevarying_suffix)

mcmc_suffix <- paste0("mcmc_summary_scenario", 
                      sim)
mcmc_list <- list.files(path = path("results", "stoch_conc_ei_normal", "mcmc_summaries"),  pattern = mcmc_suffix)

full_stan_diag <- map(mcmc_list, ~read_csv(here::here("results", "stoch_conc_ei_normal",  "mcmc_summaries", .x)) %>% 
                        mutate(address = .x)) %>%
  bind_rows() %>%
...
write_csv(full_stan_diag, here::here("results", "stoch_conc_ei_normal", paste0("stoch_ei_normal_scenario", sim,  "_allseeds_stan_diag.csv")))
...
timevarying_quantiles <- map(timevarying_list, ~read_csv(here::here("results", "stoch_conc_ei_normal", "generated_quantities", .x)) %>% 
                                 mutate(address = .x,
...
write_csv(rt_quantiles, here::here("results", "stoch_conc_ei_normal", paste0("stoch_ei_normal_scenario", sim,  "_allseeds_rt_quantiles.csv")))
...
ggsave2(filename = here::here("results", "stoch_conc_ei_normal", paste0("stoch_ei_normal_rt_plots_scenario", sim, ".pdf")),
        plot = rt_quantiles %>%
          distinct(seed) %>%
          arrange(seed) %>%
          pull(seed) %>%
